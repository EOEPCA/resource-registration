...
def get_scene_id_folder(scene_id, folder_format=None):
    ...
    if scene_id.startswith('S3'):
        ...
        if 'product' in variables:
            variables['product'] = variables['instrument'][0:2] + '_' + variables['processingLevel'] + '_' + variables['product'] + '___'
...
def validate_integrity(scene_path, scene_id):
    ...
    for f in root.findall('.//byteStream'):
        ...
        if size != expected_file_size:
            raise Exception("File %s does not match expected file size: %s vs. %s" % (check_file, size, expected_file_size))
        ...
        if checksum != expected_checksum:
            raise Exception("File %s does not match expected checksum (%s): %s vs. %s" % (check_file, checksum_alg, size, expected_checksum))
...
def modify_s2_stac(stac_item: pystac.item.Item, base_item=None):
    ...
    for i, (current_key, target_key) in enumerate(input_dict.items()):
        ...
        if current_key in stac_item_dict["assets"]:
            ...
            log.info(f'Replacing the current Asset-Key {current_key} with the new Asset-Key {target_key}: Current key not found in metadata!')
